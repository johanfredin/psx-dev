...
void CdReadFile(char *file_path, u_long **file, u_char *filesRead) {
    ...
    if (!didInitDs) {
        log(INFO, "LIBDS not initialized, run cdOpen() first");
        return;
    }
    ...
    log(INFO, "Loading file from CD: %s", file_path_raw);
    ...
    if (temp_file_info->size > 0) {
        ...
        log(DEBUG, "...file size: %lu", temp_file_info->size);
        *sectors_size = temp_file_info->size + (SECTOR % temp_file_info->size);
        log(TRACE, "...file buffer size needed: %d", *sectors_size);
        log(TRACE, "...sectors needed: %d", (*sectors_size + SECTOR - 1) / SECTOR);
